    ...
    return (
        <div className={'flex  flex-col justify-center  py-20 items-center'}>
            <div className={'max-w-[1260px] grid grid-cols-1 items-center  px-5 xl:px-0 lg:grid-cols-2 gap-20 xl:gap-10 w-full'}>
                {props.direction === "right" && props.image && <img data-tina-field={tinaField(props, 'image')} alt={props.image} src={props.image} className={'rounded-xl w-full max-h-[500px] object-cover h-full'} />}
                ...
                {props.direction === "left" && props.image && <img data-tina-field={tinaField(props, 'image')} alt={props.image} src={props.image} className={'rounded-xl w-full max-h-[500px] object-cover h-full'} />}
            </div>
        </div>
    )
}